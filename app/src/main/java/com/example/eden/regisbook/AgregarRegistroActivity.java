    ...
    private void imagePickDialog(){
        ...
        builder.setItems(options, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (which==0){
                    //click en camara
                    if (!checkCameraPermission()){//verifica el permiso
                        requestCameraPermission();
                    }
                    else{//te lleva ala camara
                        // permiso ya otorgado
                        PickFromCamera();
                    }

                }
                else if (which==1){// segunda op galeria//verifica el permiso
                    if (!checkStoragePermission()){
                        requestStoragePermission();
                    }
                    else{
                        // permiso ya otorgado/te lleva ala galeria
                        PickFromGallery();
                    }
                }
            }
